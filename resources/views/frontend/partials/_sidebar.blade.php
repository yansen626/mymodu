
<!-- Side Navigation Menu -->
<aside class="side-navigation-wrapper enter-right" data-no-scrollbar data-animation="push-in">
    <div class="side-navigation-scroll-pane">
        <div class="side-navigation-inner">
            <div class="side-navigation-header">
                <div class="navigation-hide side-nav-hide">
                    <a href="#">
                        <span class="icon-cancel medium"></span>
                    </a>
                </div>
            </div>
            <nav class="side-navigation nav-block">
                <ul>
                    <li>
                        <a href="#" class="contains-sub-menu">About MODU</a>
                        <ul class="sub-menu">
                            <li>
                                <a href="{{ route('about-us') }}">About Us</a>
                            </li>
                            <li>
                                <a href="homepage-mobile-application.html">Facts</a>
                            </li>
                            <li>
                                <a href="homepage-mobile-application-2.html">Quality Policy</a>
                            </li>
                            <li>
                                <a href="homepage-platform.html">MODU in the world</a>
                            </li>
                        </ul>
                    </li>
                    <li>
                        <a href="#" class="contains-sub-menu">Our Product</a>
                        <ul class="sub-menu">
                            <li>
                                <a href="homepage-consulting.html">Aluminium Conveyors</a>
                            </li>
                            <li>
                                <a href="homepage-mobile-application.html">Stainless Steel Conveyors</a>
                            </li>
                            <li>
                                <a href="homepage-mobile-application-2.html">Gripper Conveyor</a>
                            </li>
                            <li>
                                <a href="homepage-platform.html">Alpine Conveyor</a>
                            </li>
                            <li>
                                <a href="homepage-platform.html">Accumulation Conveyor</a>
                            </li>
                            {{--<li>--}}
                                {{--<a href="homepage-platform.html">Kaiten/Sushi Conveyor</a>--}}
                            {{--</li>--}}
                        </ul>
                    </li>
                    <li>
                        <a href="#" class="contains-sub-menu">Industrial Solutions</a>
                        <ul class="sub-menu">
                            <li>
                                <a href="homepage-consulting.html">Food & Packaging</a>
                            </li>
                            <li>
                                <a href="homepage-mobile-application.html">Tea Bag Handling</a>
                            </li>
                            <li>
                                <a href="homepage-mobile-application-2.html">Carton Handling</a>
                            </li>
                            <li>
                                <a href="homepage-platform.html">Personal & Pharmaceutical</a>
                            </li>
                            <li>
                                <a href="homepage-platform.html">Paper Converting</a>
                            </li>
                            <li>
                                <a href="homepage-platform.html">Electronic & Electrical</a>
                            </li>
                            <li>
                                <a href="homepage-platform.html">Optional Handling</a>
                            </li>
                            <li>
                                <a href="homepage-platform.html">Composite Can Manufacturing</a>
                            </li>
                            <li>
                                <a href="homepage-platform.html">Automotive & Machine Parts</a>
                            </li>
                            <li>
                                <a href="homepage-platform.html">Puck Handling</a>
                            </li>
                            <li>
                                <a href="homepage-platform.html">Tobacco Handling</a>
                            </li>
                        </ul>
                    </li>
                    <li>
                        <a href="{{ route('contact-us') }}">Contact Us</a>
                    </li>
                    <li>
                        <a href="blog-wide.html">Videos</a>
                    </li>
                </ul>
            </nav>
            <div class="side-navigation-footer">
                <p class="copyright no-margin-bottom"> Copyright &copy; 1995 - 2018 MODU System .</p>
            </div>
        </div>
    </div>
</aside>
<!-- Side Navigation Menu End -->